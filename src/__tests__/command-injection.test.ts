...
describe("Source code injection safety", () => {
  ...
  it("kiwoom-cli.ts uses execFile with an argument array, never execSync/exec with interpolation, and never constructs an order-submission command", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
      fileURLToPath(new URL("../trading/kiwoom-cli.ts", import.meta.url)),
      "utf-8",
    );
    expect(source).not.toMatch(/execSync\s*\(|(?<!execFile)exec\s*\(\s*`/);
    expect(source).toMatch(/execFile\s*\(/);
    // No path in this module may ever build a real order-submission subcommand.
    expect(source).not.toMatch(/["']orders["']/);
    expect(source).not.toMatch(/--confirm/);
  });

  it("cloudflare-tunnel.ts uses spawn with an argument array, never shell string interpolation", async () => {
    const fs = await import("fs");
    const source = fs.readFileSync(
      fileURLToPath(new URL("../trading/cloudflare-tunnel.ts", import.meta.url)),
      "utf-8",
    );
    expect(source).not.toMatch(/execSync\s*\(|exec\s*\(\s*`/);
    expect(source).toMatch(/spawn\s*\(\s*"cloudflared"/);
  });
});
